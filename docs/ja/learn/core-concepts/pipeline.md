...
graph TD
    ...
    ExecutionContext["ExecutionContext<br>(リクエスト 컨텍스트 生成)"]
    GlobalPre["Global Interceptor.PreHandle<br>(라우팅 전 전処理)"]
    Router["Router<br>(HandlerMeta 任意)"]
    ParamMeta["ParameterMeta Builder<br>(파라미터 메타情報 구성)"]
    ArgResolver["ArgumentResolver Chain<br>(인자 값 生成)"]
    RoutePre["Route Interceptor.PreHandle<br>(ルート 전処理)"]
    Invoker["Invoker<br>(Controller メソッド 呼び出し)"]
    ReturnHandler["ReturnValueHandler<br>(반환값 → レスポンス 변환)"]
    PostHook["PostExecutionHook<br>(イベント 発行 등)"]
    RoutePost["Route Interceptor.PostHandle ↩<br>(ルート 후処理)"]
    GlobalPost["Global Interceptor.PostHandle ↩<br>(グローバル 후処理)"]
    RouteAfter["Route Interceptor.AfterCompletion ↩<br>(항상 実行)"]
    GlobalAfter["Global Interceptor.AfterCompletion ↩<br>(항상 実行)"]
...
func (s *Server) handle(c echo.Context) error {
    ...
    if err := s.pipeline.Execute(ctx); err != nil {
        ...
        // 파이프라인 내부에서 이미 レスポンス이 작성되었으므로
        // Echo 기본 エラー 핸들러로 중복 伝達하지 않는다.
        return nil
    }
    return nil
}
...
type HandlerMeta struct {
    ...
    Interceptors   []Interceptor   // ルート レベル 인터셉터
}
```
...
// Controller:
func GetPost(userId path.Int, postId path.Int) // ✓ 順序 일치
```
...
type ArgumentResolver interface {
    // 이 Resolver가 해당 型을 処理할 수 있는지 판단
...
```go
// ルート Interceptor AfterCompletion (defer - 항상 実行)
defer func() {
    for i := len(routeInterceptors) - 1; i >= 0; i-- {
...
// グローバル Interceptor AfterCompletion (defer - 항상 実行)
defer func() {
    for i := len(p.interceptors) - 1; i >= 0; i-- {
...
func (p *Pipeline) Execute(ctx core.ExecutionContext) (finalErr error) {
    ...
    // グローバル Interceptor AfterCompletion (항상 実行)
    globalMeta := core.HandlerMeta{}
    defer func() {
        for i := len(p.interceptors) - 1; i >= 0; i-- {
            p.interceptors[i].AfterCompletion(ctx, globalMeta, finalErr)
    ...
    // ルート Interceptor AfterCompletion (항상 実行)
    defer func() {
        for i := len(routeInterceptors) - 1; i >= 0; i-- {
            routeInterceptors[i].AfterCompletion(ctx, meta, finalErr)
        }
    ...
    // 9. ルート Interceptor PostHandle (역순)
    for i := len(routeInterceptors) - 1; i >= 0; i-- {
        routeInterceptors[i].PostHandle(ctx, meta)
    }

    // 10. グローバル Interceptor PostHandle (역순)
    for i := len(p.interceptors) - 1; i >= 0; i-- {
        p.interceptors[i].PostHandle(ctx, meta)
    }
