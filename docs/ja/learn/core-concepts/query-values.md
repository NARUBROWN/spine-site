...
q.Int("page", 1)    // 1 (만약 page=abc라면 → デフォルト値)
```
...
q.GetBoolByKey("active", false)    // false (만약 active=maybe → デフォルト値)
```
...
// ❌ struct 태그의 デフォルト値은 非表示
type Params struct {
    Page int `query:"page" default:"1"`  // 어디서 設定됐는지 追跡困難
}
```
...
// ❌ 自動 バインディング은 zero value와 "값 없음"을 구분 못함
type Params struct {
    Premium bool `query:"premium"`  // false가 デフォルト値인지 明示적 false인지?
}
```
