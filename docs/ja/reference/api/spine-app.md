...
type CustomTransport interface {
    // Init은 DI Container 준비 이후 呼び出し됩니다.
    Init(container Container) error
    // Start는 Init 이후 별도 goroutine에서 呼び出し됩니다.
    Start() error
    // Stop은 Graceful Shutdown 시 呼び出し됩니다.
...
app.Consumers().Register(
    "order.created",            // 토픽/イベント 名前
    (*OrderConsumer).OnCreated, // 핸들러 メソッド
)
