...
> **注意**：`internal/event/publish.EventBus` 是 `core.EventBus` 的類型別名 (`type EventBus = core.EventBus`)，其內部實作設定為滿足此類型。
...
type ExecutionContext interface {
    ...
    Header(name string) string         // HTTP 標頭（Consumer、WS 為空字串）
    ...
    // 內部儲存
    Set(key string, value any)         // 儲存值
    Get(key string) (any, bool)        // 查詢值
}
```
...
func (e *echoContext) Context() context.Context {
    return e.reqCtx  // HTTP 請求的 context
}
```

#### EventBus()

回傳請求範圍的 EventBus，用於在 Controller 中發布領域事件。
...
type HttpRequestContext interface {
    ...
    // 單獨的參數訪問
    Param(name string) string          // 指定 path param
    Query(name string) string          // 指定 query param（第一個值）
    Header(name string) string         // 指定標頭
    
    // 全視圖訪問
    Params() map[string]string         // 所有 path params
    Queries() map[string][]string      // 所有 query params
    Headers() map[string][]string      // 所有標頭
...
func (r *DTOResolver) Resolve(ctx core.ExecutionContext, parameterMeta ParameterMeta) (any, error) {
    ...
    if !ok {
        return nil, fmt.Errorf("不是 HTTP 請求 context")
    }

    valuePtr := reflect.New(parameterMeta.Type)

    if err := httpCtx.Bind(valuePtr.Interface()); err != nil {
        return nil, fmt.Errorf("DTO 綁定失敗 (%s): %w", parameterMeta.Type.Name(), err)
    }
...
type ConsumerRequestContext interface {
    ...
    EventName() string    // 事件名稱（例如："order.created")
    Payload() []byte      // 事件 Payload（JSON 等）
}
```
...
type WebSocketContext interface {
    ...
    ConnID() string       // 連線 ID
    MessageType() int     // 訊息類型（Text、Binary 等）
    Payload() []byte      // 訊息 Payload
}
```
...
type echoContext struct {
    echo     echo.Context           // Echo 的原始 context
    reqCtx   context.Context        // 請求範圍 context
    store    map[string]any         // 內部儲存
    eventBus publish.EventBus       // 事件匯流排
}
...
回傳副本以防止對原始地圖進行外部更改。使用 `maps.Copy`。
...
func (r *PathIntResolver) Resolve(ctx core.ExecutionContext, parameterMeta ParameterMeta) (any, error) {
    ...
    if !ok {
        return nil, fmt.Errorf("找不到 path param. %s", parameterMeta.PathKey)
    }
...
func (r *EventNameResolver) Resolve(ctx core.ExecutionContext, meta resolver.ParameterMeta) (any, error) {
    ...
    if name == "" {
        return nil, fmt.Errorf("EventName 無法在 RequestContext 中找到")
    }
...
func (r *DefaultRouter) Route(ctx core.ExecutionContext) (core.HandlerMeta, error) {
    ...
    return core.HandlerMeta{}, httperr.NotFound("找不到處理程式.")
}
```
    ...
    for _, paramMeta := range paramMetas {
        ...
        if !resolved {
            return nil, fmt.Errorf(
                "ArgumentResolver 沒有 parameter. %d (%s)",
                paramMeta.Index,
...
func (h *JSONReturnHandler) Handle(value any, ctx core.ExecutionContext) error {
    rwAny, ok := ctx.Get("spine.response_writer")
    if !ok {
        return fmt.Errorf("在 ExecutionContext 中找不到 ResponseWriter.")
    }
    
    rw, ok := rwAny.(core.ResponseWriter)
    if !ok {
        return fmt.Errorf("ResponseWriter 類型不正確.")
    }
...
### 2. Resolver 接收 ExecutionContext，並斷言為所需類型
...
httpPipeline.AddArgumentResolver(
    &resolver.StdContextResolver{},           // 共用
    &resolver.ControllerContextResolver{},    // 共用
    &resolver.HeaderResolver{},               // HTTP 專用
    &resolver.PathIntResolver{},              // HTTP 專用
    &resolver.PathStringResolver{},           // HTTP 專用
    &resolver.PathBooleanResolver{},          // HTTP 專用
    &resolver.PaginationResolver{},           // HTTP 專用
    &resolver.QueryValuesResolver{},          // HTTP 專用
    &resolver.DTOResolver{},                  // HTTP 專用
    &resolver.FormDTOResolver{},              // HTTP 專用
    &resolver.UploadedFilesResolver{},        // HTTP 專用
)

// 消費管道
consumerPipeline.AddArgumentResolver(
    &resolver.StdContextResolver{},           // 共用
    &eventResolver.EventNameResolver{},       // Consumer 專用
    &eventResolver.DTOResolver{},             // Consumer 專用
)

// WebSocket 管道
wsPipeline.AddArgumentResolver(
    &resolver.StdContextResolver{},           // 共用
    &wsResolver.ConnectionIDResolver{},       // WebSocket 專用
    &wsResolver.DTOResolver{},                // WebSocket 專用
)
```
