...
graph TD
    ...
    ExecutionContext["ExecutionContext<br>(建立請求上下文)"]
    GlobalPre["Global Interceptor.PreHandle<br>(路由前预處理)"]
    Router["Router<br>(選擇 HandlerMeta)"]
    ParamMeta["ParameterMeta Builder<br>(构建參數元資料)"]
    ArgResolver["ArgumentResolver Chain<br>(生成參數值)"]
    RoutePre["Route Interceptor.PreHandle<br>(路由预處理)"]
    Invoker["Invoker<br>(调用 Controller 方法)"]
    ReturnHandler["ReturnValueHandler<br>(將回傳值转换為回應)"]
    PostHook["PostExecutionHook<br>(發布事件等)"]
    RoutePost["Route Interceptor.PostHandle ↩<br>(路由后處理)"]
    GlobalPost["Global Interceptor.PostHandle ↩<br>(全域后處理)"]
    RouteAfter["Route Interceptor.AfterCompletion ↩<br>(始终執行)"]
    GlobalAfter["Global Interceptor.AfterCompletion ↩<br>(始终執行)"]
    ErrorHandler["handleExecutionError<br>(錯誤安全网)"]
...
**在路由之前**先運行全域攔截器。此時尚未確定將執行哪個處理程式，因此傳遞了一個空的 `HandlerMeta` 。
...
func (r *DefaultRouter) Route(ctx core.ExecutionContext) (core.HandlerMeta, error) {
    ...
    return core.HandlerMeta{}, httperr.NotFound("找不到處理器.")
}
```
...
type HandlerMeta struct {
    ControllerType reflect.Type    // 控制器型別
...
路由之後，路由等級攔截器在Controller呼叫之前執行。此攔截器包含在 `HandlerMeta.Interceptors` 中，並且僅適用於該特定處理程式。
...
func (p *Pipeline) handleReturn(ctx core.ExecutionContext, results []any) error {
    ...
    for _, result := range results {
        ...
        if _, isErr := result.(error); isErr {
            ...
            return fmt.Errorf(
                "沒有 ReturnValueHandler 處理 error 回傳值. (%s)",
                resultType.String(),
            )
        }
    }
...
### ReturnValueHandler 介面

```go
// 內部/處理程式/return_value.go
...
Pipeline 同樣用於 Single HTTP Pipeline、Consumer Pipeline 和 WebSocket Pipeline。每個Transport都會建立一個單獨的Pipeline實例，只有Resolver和Handler設定不同。
